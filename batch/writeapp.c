...
extern CHAR_T      *RegisteredAppEvents[];
...
/*----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------*/
#ifdef __STDC__
static void         WriteIncludes (CHAR_T* FName, FILE * file)
#else  /* __STDC__ */
...
#  ifdef _I18N_
   ...
   wcstombs (fname, FName, MAX_TXT_LEN);
#  else  /* !_I18N_ */
   char* fname = FName;
#  endif /* !_I18N_ */
...
/*----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------*/
#ifdef __STDC__
static void         WriteAnEvent (CHAR_T *eventName, PtrActionEvent pactevent)
#else  /* __STDC__ */
...
#  ifdef _I18N_
   wcstombs (mbsTmpStr, eventName, 1024);
...
#endif /* __STDC__ */
{
   ...
   ustrcpy (name, TEXT("Tte"));
   for (event = 0; event <= TteExit; event++)
     {
	ustrcpy (&name[3], RegisteredAppEvents[event]);
...
#      ifdef _I18N_
	{
     wcstombs (mbsTmpStr, subitem->AppItemName, 1024);
     fprintf (AppFile, ", %s", mbsTmpStr);
	}
#      else  /* !_I18N_ */
...
#      ifdef _I18N_
	{
       wcstombs (mbsTmpStr, subitem->AppItemActionName, 1024);
	   fprintf (AppFile, ", \"%s\", '%c');\n", mbsTmpStr, (char)subitem->AppItemType);
	}
#      else  /* !_I18N_ */
...
/*----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------*/
#ifdef __STDC__
static void         PrintMenus (PtrAppMenu firstMenu, WindowType winType, CHAR_T *schemaName)
...

{
   ...
   PtrAppMenuItem      item;
   int                 itemsNumber;
   char                schName[MAX_TXT_LEN];

   wc2iso_strcpy (schName, schemaName);
   if (firstMenu == NULL)
     {
...
#        ifdef _I18N_
	     ...
	     fprintf (AppFile, ", TEXT(\"%s\"), %d, %s, %d", schName, menu->AppMenuView, mbsTmpStr, itemsNumber);
#        else  /* !_I18N_ */
...
#                  ifdef _I18N_
			   {
                   wcstombs (mbsTmpStr, item->AppItemActionName, 1024);
                   fprintf (AppFile, ", \"%s\", '%c');\n", mbsTmpStr, (char)item->AppItemType);
			   }
#                  else  /* !_I18N_ */
...
/*----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------*/
#ifdef __STDC__
static void         WriteAppliInit (CHAR_T* FName, PtrEventsSet pAppli)
...
#  ifdef _I18N_
   wcstombs (mbsTmpStr, pAppli->EvSName, 1024);
   fprintf (AppFile, "  appliActions = TteNewEventsSet (%d, TEXT(\"%s\"));\n", pAppli->EvSStructId, mbsTmpStr);
#  else  /* !_I18N_ */
...
/*----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------*/
#ifdef __STDC__
static void         WriteActionList (CHAR_T* FName)
#else  /* __STDC__ */
...
#  endif /* !_I18N_ */
   ...
   wc2iso_strcpy (actionFname, FName);
   ...
   if (SchemasUsed != NULL)
	...
	while (SchUsed != NULL)
	  {
#        ifdef _I18N_
         wcstombs (mbsTmpStr, SchUsed->AppNameValue, 1024);
...
#           ifdef _I18N_ 
            {
            wcstombs (mbsTmpStr, menuAction->AppNameValue, 1024);
...
#        endif /* !_I18N_ */
	...
	fprintf (AppFile, "\n");
	PrintMenus (MainWindowMenus, MainWindow, TEXT("MAIN"));

	fprintf (AppFile, "\n");
	PrintMenus (DocWindowMenus, DocWindow, TEXT("DOC"));
...
#      ifdef _I18N_
       {   
       wcstombs (mbsTmpStr, pSSchema->SsRule[i].SrName, 1024);
...
#      endif /* !_I18N_ */
   ...
   while (name != NULL)
     {
#   ifdef _I18N_
    wcstombs (mbsTmpStr, name->AppNameValue, 1024);
	fprintf (dotHFile, "#define %s  %d\n", mbsTmpStr, num);
...
/*----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------*/
#ifdef __STDC__
void                GenerateApplication (CHAR_T *fname, PtrEventsSet pAppli)
#else  /* __STDC__ */
...
#endif /* __STDC__ */
{
   ...
   i = 0;
   fileSuffix = TtaWCSdup (fname);

   /* met le suffixe APP.c a la fin du nom de fichier */
   while (fname[i] != TEXT(' ') && fname[i] != TEXT('\0'))
      i++;
   fname[i] = TEXT('\0');
   ustrcpy (&fname[i], TEXT("APP.c"));

   /* cree le fichier */
   AppFile = ufopen (fname, TEXT("w"));
   ...
   if (ustrcmp (TEXT("EDITOR"), fileSuffix) == 0)
     {
