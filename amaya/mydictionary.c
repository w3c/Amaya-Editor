...
Record Find(DicDictionary dic, DicKey key) {
	Record rec = dic->first;
...
/*----------------------------------------------------------------------
  Looks up dic for the element identified by key, returns :
  ...
  ----------------------------------------------------------------------*/
Record FindPrevious(DicDictionary dic, DicKey key, bool& isFirst) {
	Record rec = dic->first;
	Record precedent = NULL;
...
DicElement Add(DicDictionary dic, DicKey key, DicElement el) {	
	...
	if(!rec) { //The element should be the first
	...
	else if(strcmp(rec->key, key)==0) { //The element has been found
		result          = rec->element; //We return the old element
		rec->element    = el; //And use the new one
	}

	else { //The element should be insered just after rec
		newRec          = (Record) malloc(sizeof(sRecord));
		newRec->key     = (char *) strdup(key);
		newRec->element = el;
...
DicElement Remove(DicDictionary dic, DicKey key) {
	...
	else if(!rec) //The element is not in the dictionary
		return NULL; 
...
DicDictionary CreateDictionaryFromList(char *list) {
	...
	for(unsigned int i=0; i<strlen(list); i++) {		
		...
		while(isEOSorWhiteSpace(list[i])) ++i;
