...
/*   candidat = match_env.TabChild[p][0]; */
   ...
   while (!matchfailed && !matchfound) {
	 ...
	 } else {		/* un autre candidat n'a pas encore pu etre essaye */
	    candidat = ms->next;
	 }
      } else {			/* inv :  child!=NULL && candidat != -1 */
...
#endif
{
   ...
   if (strcmp (bufHTML, "")) {
	 ...
	 if (invEl != NULL) {
	    ...
	    || ((MyLastSelect && TtaIsBefore (invEl, MyLastSelect)))) {	/* if the transformation failed */
	       res = FALSE;
	       while (courEl != NULL && courEl != MyLastSelect) {	/* deleting the generated structure */
		  lastEl = courEl;
		  TtaNextSibling (&courEl);
		  TtaDeleteTree (lastEl, doc);
	       ...
	       while (DMatch != NULL) {		/* restoring the source elements */
	 ...
	 if (invEl != NULL) {
	    if (MyLastSelect == NULL || TtaIsBefore (invEl, MyLastSelect)) {	/* if the transformation failed */
	       res = FALSE;
...
#endif
{
   ...
   *nbcreated = 0;
   if (TtaIsLeaf (TtaGetElementType (elSource))) {	/* if the source element is a leaf, it is impossible to flatten it more */
      LastCreated = NULL;
   ...
   if (voisin)
      elPreced = elDest;
   while (elCour != NULL && res) {	/* for each child of the source element */
      elNext = elCour;
...
#endif
{
   ...
   AtType.AttrSSchema = TtaGetDocumentSSchema (doc);
   AtType.AttrTypeNum = HTML_ATTR_Ghost_restruct;
   while (courEl != NULL && courEl != lastEl) {/* pour chaque elt genere */
...
#endif
{
   ...
   while (AD != NULL) {
      if (AD->IsTransf) {	/* transfer attribute */
	 ancestor = TN;
	 found = FALSE;
	 while (!found && ancestor != NULL) {	/* searching for source element (in current element ancestors) */
	    found = (!strcmp (ancestor->transsymb->SName, AD->AttrTag)
		     || !strcmp (ancestor->transsymb->Tag, AD->AttrTag));
	    if (!found)
	       ancestor = ancestor->parent;
	 }
	 if (found) {		/* searching for an ancestor of the source element which have the wanted attribute  */
	    if (ancestor != NULL) {
	 ...
	 if (!found) {
      ...
      } else {			/* creation of an attribute */
	 PutInHTMLBuf (" ");
	 PutInHTMLBuf (AD->NameAttr);
	 PutInHTMLBuf ("=");
	 if (AD->IsInt) {	/* int attribute */
...
#endif
{
   ...
   child = node->child;
   while (child != NULL) {
      if (TtaGetElementVolume (child->element) != 0) {	/* if the element is empty: no transfert */
	 pilenode[toppilenode]->nbc++;
...
#endif
{
   if (TtaGetElementVolume (node->element) != 0) {	/* if the element is empty: no transfert */
      if (!inplace)
...
#endif
{
   ...
   while (smc2 != NULL) {
	 ...
	 } else {		/* il n'y a pas de matching pour les fils de ce noeud */
	    TransfertNode (sm->node, FALSE);
	 }
      } else {			/* il existe une regle de transformation pour ce noeud */
...
#endif
{
   ...
   while (!stop) {		/* pour chaque noeud optionnel */
      if (!strcmp (pilenode[cournode]->Tag, RNodeCour->Tag)) {	/* le tag est deja present : on ne genere rien */
   ...
   while (toppilenode >= cournode) {	/* on ferme les tags de la pile (crees par la regle precedente) */
      PutEndTag (pilenode[toppilenode]);
      TtaFreeMemory (pilenode[toppilenode]->Tag);
      TtaFreeMemory ((char *) pilenode[toppilenode]);
      toppilenode--;
   ...
   while (RNodeCour != NULL) {	/* on genere des noeuds optionnels non deja presents */
...
/*       pilenode[++toppilenode]=RNodeCour; */
   ...
   while (RNodeCour != NULL && strcmp (RNodeCour->Tag, "*")) {	/* on genere les nouveaux noeuds */
      PutBeginTag (RNodeCour, sm->node);
/*       pilenode[++toppilenode]=RNodeCour; */
      cournode++;
      RNodeCour = RNodeCour->next;
   ...
   else if (sonsmatch) {	/* si les fils ont ete matches */
      ApplyTransChild (sm2->childmatches);
   } else {			/*Les fils ne matchent pas : on les transfere */
...
#endif
{
   ...
   if (res) {			/* if the parsing was succesful */
...
#endif
{
   ...
   if (MyFirstSelect != MyLastSelect) {
      if (MyFirstSelect != NULL && ffc <= 1) {	/* searching for the first selected element */
	 ...
	 while (parentFirst != NULL && prevFirst == NULL && TtaIsBefore (parentFirst, MyLastSelect)) {
      ...
      if (MyLastSelect != NULL && (llc == 0 || (llc > 0 && llc >= TtaGetTextLength (MyLastSelect)))) {	/* searching for the last selected element */
   ...
   if (result && parentFirst != NULL) {		/* if all selected elements are at the same level, checking if ancestors have any sibling */
...
#endif
{
   ...
   if (*elselect == NULL || *elselect == MySelect || *elselect == MyLastSelect) {	/* if the selection is an unique element, or elselect is the last */
      *elselect = NULL;
   } else {
...
#endif
{
   ...
   while (result && smc != NULL) {
      if (smc->patsymb->Rule == NULL) {		/* there is no rule for the current node */
	 sm2 = smc->node->matches;
	 sonsmatch = FALSE;
	 while (sm2 != NULL && !sonsmatch) {	/* checks if the children of the node have been matched */
	    sonsmatch = (sm2->patsymb == smc->patsymb && sm2->childmatches != NULL);
	    if (!sonsmatch)
	       sm2 = sm2->next;
	 }
	 if (!sonsmatch) {	/* if the children of the node have not been matched */
	    /* checks if the node can be transferred in the destination */
	    if (TtaGetElementVolume (smc->node->element) != 0) {	/* if the element is empty, it is ignored in transformation */
	 ...
	 } else {		/* if they have been, checks the elements generated by these children */
	    result = CheckValidTransRoot (sm2, elTypeRoot, prevTag);
	 }
      } else {			/* there is a rule for the current node */
	 node = smc->patsymb->Rule->OptNodes;
	 if (node != NULL) {	/* if there is at least one place node */
...
#endif
{
   ...
   if (CheckSelectionLevel (TransDoc) && ppStartParser ("HTML")) {
	 ...
	 while (elemselect != NULL) {
      ...
      j = 0;
      tag = TtaGetMemory (NAME_LENGTH);
      do {			/* for each node above the first selected */
	 sm = node->matches;
	 while (sm != NULL) {	/* for each matching of the node */
	    if (!strcmp (sm->patsymb->Tag, "pattern_root")) {	/* if it is matching a pattern root : insert the transformation name */
	       /* in the menu buffer */
	       strcpy (tag, "");
	 ...
	 node = node->child;
      ...
      TtaFreeMemory (tag);
      if (i > 0) {		/* if some transformations have been matched, shows the menu */
...
#endif
{
   ...
   if (CheckSelectionLevel (TransDoc) && ppStartParser ("HTML")) {
      ...
      while (td != NULL) {
	 if (td->TagDest == NULL || strcmp (td->TagDest, DestTag)) {	/* the transformation does not produce the asked tag, it is desactived */
	 ...
	 do {			/* for each node above the first selected */
	    ...
	    sm = node->matches;
	    while (sm != NULL) {	/* for each matching of the node */
	       if (!strcmp (sm->patsymb->Tag, "pattern_root")) {	/* if it is matching a pattern root : insert the transformation */
