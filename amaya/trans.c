...
#define InBuffer 1
static char*         BufSource;		/* HTML text buffer */
static int           SzSource;		/* size of generated HTML code */
      ...
      if (inputFile)
        {
          ...
          while (charRead != EOF && SzSource < BUFFER_LEN - 1)
            {
              BufSource[SzSource++] = charRead;
              charRead = getc (inputFile);
            }
  ...
  if (BufSource[0] == EOS)
    {
      ...
      if (DocumentMeta[doc]->xmlformat)
        ParseXmlBuffer (BufSource, myFirstSelect, isClosed, doc, language, NULL);
      else
        ParseSubTree (BufSource, myFirstSelect, language, isClosed, doc);
          ...
          if (RNodeCour != NULL && RNodeCour->Tag[0] == '"')
            {
              ...
              strncpy ((char *)&BufSource[SzSource], (char *)&RNodeCour->Tag[1], l);
              SzSource += l;
              BufSource[SzSource] = EOS;
            }
          if (RNodeCour != NULL && RNodeCour->Tag[0] == '*')
  ...
  if (sm->MatchChildren)
    {
      ...
      lastRulePlace = 1;
      SzSource = 0;
      BufSource = (char *)TtaGetMemory (BUFFER_LEN);
      BufSource[0] = EOS;
