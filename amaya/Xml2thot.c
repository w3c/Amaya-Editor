...
#define MAX_XMLRootName   50
static char          XMLRootName[MAX_XMLRootName];
...
static  int         SUBTREE_ROOT_LEN = 18;
...
static void       StartOfXmlStartElement (char *name)
{
  ...
  if (currentParserCtxt != NULL && !UnknownElement)
      ...
      if (UnknownNS)
	{
      ...
      else if (strlen (elementName) > NAME_LENGTH)
	{
	  /* The element name is greater than the Thot limit */
	  sprintf (msgBuffer, 
		   "Element name too long for Amaya <%s>", elementName);
	  XmlParseError (errorParsing, msgBuffer, 0);
	  UnknownElement = TRUE;
	}
      else
	{
	  ...
	  if (mappedName == NULL)
		...
		strcpy (schemaName, currentParserCtxt->SSchemaName);
	      if (strncmp (elementName, "SUBTREE_ROOT", SUBTREE_ROOT_LEN))
		{
			       ...
			       const XML_Char **attlist)
{
   ...
   if (XMLcontext.parsingTextArea)
     ...
     {     
       ...
       if (strncmp ((char*) name, SUBTREE_ROOT, SUBTREE_ROOT_LEN) != 0)
	 {
	   ...
	   if (XMLRootName[0] == EOS)
	     ...
	     strncpy (XMLRootName, (char*) name, MAX_XMLRootName);
	 }
     }
}
...

{
   ...
   if ((strncmp ((char*) name, SUBTREE_ROOT, SUBTREE_ROOT_LEN) != 0) && 
       (XMLRootName[0] != EOS) && !XMLrootClosed)
     {
       ...
       if (!strncmp (XMLRootName, (char*) name, MAX_XMLRootName) && stackLevel == 1)
	 {
	   XMLrootClosed = TRUE;
	 }
     }
