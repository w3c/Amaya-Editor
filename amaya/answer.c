 ...
 if (request && HTRequest_internal(request)) return NO;
    ...
    switch (op) {
		 ...
		 TtaSetStatus(me->docid, 1, TtaGetMessage(AMAYA, AM_PROG_READ), tempbuf);
	       } else TtaSetStatus(me->docid, 1, TtaGetMessage(AMAYA, AM_PROG_READ), me->urlName);
	     }
	     break;
            case HT_PROG_WRITE:
	      if ((me->method == METHOD_PUT) || (me->method == METHOD_POST)) {
	...
	break;
           default:           TtaSetStatus(me->docid,1, TtaGetMessage(AMAYA, AM_UNKNOWN_STATUS), NULL);
                              break;
    }
    return YES;
}
...
#endif
{
    ...
    if (input) TtaNewLabel (BaseDialog+TextLabel, BaseDialog+FormAnswer, (char*) input);

    if (dfault) TtaNewLabel (BaseDialog+TextLabel, BaseDialog+FormAnswer, (char*) dfault);
...
#endif /* __STDC__ */
{
    ...
    while ((pres = (HTError *) HTList_nextObject(cur))) {
	  ...
	  switch (index) {
		 case HTERR_UNAUTHORIZED: TtaSetStatus(me->docid,1,
...
#endif /* __STDC__ */
{
    ...
    if (WWWTRACE) HTTrace("HTError..... Generating message\n");
    if (!request || !cur) return;

    while ((pres = (HTError*) HTList_nextObject(cur))) {
          index = HTError_index(pres);
          if (HTError_doShow(pres)) { /* Error number */
	     switch (index) {
	            case HTERR_SYSTEM:
	            case HTERR_INTERNAL:
	                 if (pres->par != NULL) {
