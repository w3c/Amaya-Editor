...
DicDictionary templates = InitializeTemplateEnvironment();
...
----------------------------------------------------------------------*/
void GetTemplate_callback (int newdoc, int status, 
						   char *urlName,
						   char *outputfile, 
						   AHTHeaders *http_headers,
						   void * context)
{	
	XTigerTemplate t = NewXTigerTemplate();
...
void LoadTemplate(char* templatename, char* schemaname) {
	...
	if (templateDoc != 0)
	{
		...
		Document newdoc = GetAmayaDoc (
			templatename, NULL, templateDoc,
...
void CreateInstanceDocument(Document doc, char *templatename, char *docname, DocumentType docType) {
							  ...
							  DocumentType docType)
{
	...
	bool dontReplace = DontReplaceOldDoc;
