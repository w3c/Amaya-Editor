...
/*----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------*/
void         CheckParsingErrors (Document document)
{
  char      *profile;
  
  if (ErrFile &&
      (HTMLErrorsFound || XMLErrorsFound || CSSErrorsFound ||
       XMLErrorsFoundInProfile || XMLNotWellFormed ||
       XMLUnknownEncoding || XMLCharacterNotSupported))
    {
      /* Some warnings/errors are detected */
      ...
      else
	{
	  if (XMLNotWellFormed)
	  ...
	  else if (XMLErrorsFoundInProfile)
	    {
	      ...
	      if (!profile)
		profile = "";
	      InitConfirm3L (document, 1, TtaGetMessage (AMAYA, AM_XML_PROFILE),
