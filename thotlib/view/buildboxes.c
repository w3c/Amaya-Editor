...
#ifdef DEBUG_MAC
if (box->DisplayList)
 printf ( "UnsplitBox : glIsList=%s (pose prb sur certaines machines)\n", glIsList (box->DisplayList) ? "yes" : "no" );
#endif /* DEBUG_MAC */
...
/*----------------------------------------------------------------------
  ...
  pNextBox points to the terminal box that follows the created box
  ----------------------------------------------------------------------*/
      ...
      if (pAb->AbFirstEnclosed == NULL)
        {
	      ...
	      // Get the previous displayed box
	      if (pNextBox)
		box = pNextBox->BxPrevious;
	      else
	      ...
	      if (pNextBox)
		{
		  pNextBox->BxPrevious = pBox;
                  if (pNextBox->BxType == BoPiece || pNextBox->BxType == BoScript)
                    /* update also the split parent box */
                    pNextBox->BxAbstractBox->AbBox->BxPrevious = pBox;
		}
              else
		//Insert at the end
          ...
          while (pChildAb)
            {
              box = CreateBox (pChildAb, frame, inlineChildren,
			       inlineFloatC, carIndex, incfloat, pNextBox);
              pChildAb = pChildAb->AbNext;
            }
      ...
      if (pAb->AbNew)
        {
          ...
          pBox = CreateBox (pAb, frame, inLine, inLineFloat, &i, incfloat, pNextBox);
