...
static ThotBool HasVariableWidth (PtrBox pBox, PtrBox pBlock)
{
  ...
  return (pAb->AbLeafType == LtCompound &&
          (pAb->AbWidth.DimUnit == UnPercent || pAb->AbWidth.DimUnit == UnAuto) &&
          pAbRef &&  pAbRef->AbBox &&
  ...
  if (pBlock->BxContentWidth || pBlock->BxShrink)
  ...
  else if (rtl)
    {
      ...
      if (pLine->LiRealLength > pLine->LiXMax)
      ...
      else
        {
          ...
          else if (pBlock->BxAbstractBox->AbAdjust == AlignCenter)
            delta = (pLine->LiXMax - pLine->LiRealLength) / 2;
          else if (pBlock->BxAbstractBox->AbAdjust == AlignLeft)
            delta = pLine->LiXMax - pLine->LiRealLength;
  ...
  else
    {
      if (pBox == pLine->LiLastBox && pBox->BxAbstractBox &&
        ...
        delta = (pLine->LiXMax - pLine->LiRealLength) / 2;
      else if (pBlock->BxAbstractBox->AbAdjust == AlignRight)
        delta = pLine->LiXMax - pLine->LiRealLength;
          ...
          if ((pLine->LiYOrg + orgY >= by &&
                ...
                pLine->LiYOrg + pLine->LiHeight + orgY <= by + bh))
            {
              ...
              if (floatL->BxLMargin > 0)
                bw -= floatL->BxLMargin;
              pLine->LiXOrg = floatL->BxXOrg + bw + indent - orgX;
              bottomL = by + bh - orgY;
            }
          else if (pfloatL)
            {
              ...
              if (!extensibleBlock)
                {
                 ...
                 if (mr > 0 && setinline)
                  ...
                  l += ml + pNextBox->BxLBorder;
                  r += mr + pNextBox->BxRBorder;
                  ...
                  if (pNextBox->BxShrink)
                    {
                      ...
                      if (w > 0 && w < val)
                        ...
                        ResizeWidth (pNextBox, pBlock, pBlock,
                           ...
                           pNextBox->BxAbstractBox->AbWidth.DimAbRef ||
              ...
              PtrBox *floatL, PtrBox *floatR)
{
  ...
  if ((boxPrevL && y < boxPrevL->BxYOrg + boxPrevL->BxHeight) ||
    ...
    {
       if (bw > w + 1 && boxPrevL && boxPrevR && bottomL < bottomR)
         {
           w = shiftr - left - orgX;
           y = bottomL;
         }
       else if (bw > w + 1 && boxPrevL && boxPrevR && bottomR < bottomL)
         {
  ...
  if (box->BxAbstractBox->AbFloat == 'L' && ml < 0)
    x += ml;
  else if (box->BxAbstractBox->AbFloat == 'R' && ml < 0)
    // a negative left margin of a right float shifts the box to the right
    x -= ml;
