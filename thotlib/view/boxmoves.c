...
/*----------------------------------------------------------------------
  ...
  When the parameter withBlock is TRUE vertical spacing of ghost block are managed
  Returns
...
void GetExtraMargins (PtrBox pBox, PtrAbstractBox pFrom, int frame,
                      ThotBool withBlock, int *t, int *b, int *l, int *r)
{
                  ...
                  if ( box->BxType == BoFloatGhost && pFrom != pAb)
                  ...
                  else
                    {
                      ...
                      if (pParent->AbBox->BxType == BoBlock ||
                        ...
                        {
                          if (isBlock && withBlock && first)
                            *t += box->BxTMargin + box->BxTBorder + box->BxTPadding;
                          else if (!isBlock && pFrom != pAb)
                            *t += box->BxTBorder + box->BxTPadding;
                        }
                      if (pParent->AbBox->BxType == BoBlock ||
                          pParent->AbBox->BxType == BoFloatBlock || last)
                        {
                          if (isBlock && withBlock && last)
                            *b += box->BxBMargin + box->BxBBorder + box->BxBPadding;
                          else if (!isBlock && pFrom != pAb)
                  ...
                  int l, int r, int spaceDelta, int frame, ThotBool shrink)
{
  ...
  if (pAb == NULL)
    return;
  GetExtraMargins (pBox, NULL, frame, FALSE, &i, &j, &extraL, &extraR);
                   ...
                   int delta, int t, int b, int frame)
{
  ...
  GetExtraMargins (pBox, NULL, frame, FALSE, &extraT, &extraB, &i, &j);
  if (!pAb->AbMBPChange && delta)
    {
      if (pBox->BxTMargin > 0)
