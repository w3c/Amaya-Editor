...
ThotColor           gc_fg;
  ...
  else if (RO && ColorPixel (fg) == Black_Color)
     {
...

{
   ...
   if (pat != NULL) {
     ...
     gdk_draw_rectangle ( FrRef[frame], TtGreyGC, TRUE, 
			  x + FrameTable[frame].FrLeftMargin, y + FrameTable[frame].FrTopMargin, width-1, height-1);
...
/*----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------*/
#ifdef __STDC__
void                WChaine (GdkDrawable *w, STRING string, int x, int y, ptrfont font, ThotGC GClocal)
#else  /* __STDC__ */
...
#endif /* __STDC__ */
{
  GdkDrawable  *w;
   ...
   if (w != None)
     {
       gdk_draw_rectangle (w, TtInvertGC, TRUE, 
			   x + FrameTable[frame].FrLeftMargin, y + FrameTable[frame].FrTopMargin, width, height);
     }
}
...
/*----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------*/
#ifdef __STDC__
void                PaintWithPattern (int frame, int x, int y, int width, int height, GdkDrawable *w, int RO, int active, int fg, int bg, int pattern)
#else  /* __STDC__ */
