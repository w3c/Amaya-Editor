...
/* ---------------------------------------------------------------------- */
/* |    InitShellMsg initialise le fichier de transit des messages en   | */
/* |            provenance du background.                               | */
/* ---------------------------------------------------------------------- */
#ifdef NEW_WILLOWS
#include <fcntl.h>
#endif
...
{
   background = 0;
   sprintf (msgfile_name, "/tmp/ThotMsg%d", (int) ThotPid_get ());
   msgfile_fid = ThotFile_open (msgfile_name, ThotFile_READWRITE | ThotFile_CREATE);
}

/* ---------------------------------------------------------------------- */
/* |    RmShellMsg detruit le fichier de transit des messages.          | */
/* ---------------------------------------------------------------------- */
void                RmShellMsg ()
{
   if (msgfile_fid != ThotFile_BADHANDLE)	/* was if(msgfile_fid) but was   */
     {				/* never set to 0 - EGP          */
	ThotFile_close (msgfile_fid);
	RemoveFile (msgfile_name);
     }
}


/* ---------------------------------------------------------------------- */
/* |    GetShellMsg recupere un nouveau message dans le fichier         | */
/* |            temporaire et appel l'affichage de ce message.          | */
/* ---------------------------------------------------------------------- */
void                GetShellMsg ()
{
   int                 got_one;
   ThotFileOffset      newpos;
   char               *newline;

   /* si on a des process en background on teste pour savoir ou on en est */
   if (background == 0)
      return;

   if (msgfile_fid == ThotFile_BADHANDLE)
     {
	/* il n'y aura jamais de messages */
	background = 0;
	return;
     }
   ...
   do
     {
	newpos = ThotFile_seek (msgfile_fid, 0, ThotFile_SEEKEND);
	if (newpos != msgfile_curpos)
	  {
	     ThotFile_seek (msgfile_fid, msgfile_curpos, ThotFile_SEEKSET);
	     ThotFile_read (msgfile_fid, &msg_buffer[0], newpos - msgfile_curpos);
	     if ((newline = strchr (msg_buffer, '\n')) != NULL)
	       {
		  *newline = '\0';
		  if (got_one != 0)
		     SLEEP (2);	/* attente pour la lecture du message precedent */
		  /* Message(msg_buffer, 2); */
		  TtaDisplaySimpleMessageString (LIB, CONFIRM, LIB_STRING, msg_buffer);
		  msgfile_curpos += newline - msg_buffer + 1;
		  background -= 1;
		  got_one += 1;
	       }
	     else
