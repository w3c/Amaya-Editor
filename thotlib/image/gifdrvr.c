...
#endif /* __STDC__ */
{
   ...
   PicMask = XCreatePixmap (GDp (0), GRootW (0), w, h, 1);
   ...
   if ((PicMask == (Pixmap) None) || (newmask == NULL))
	...
	PicMask = None;
     ...
     {
	tmp_gc = XCreateGC (GDp (0), PicMask, 0, NULL);
	XPutImage (GDp (0), PicMask, tmp_gc, newmask, 0, 0, 0, 0, w, h);
	XDestroyImage (newmask);
...
/* ---------------------------------------------------------------------- */
#ifdef __STDC__
void                GifPrintImage (char *fn, PictureScaling pres, int xif, int yif, int wif, int hif, int PicXArea, int PicYArea, int PicWArea, int PicHArea, int fd, unsigned long BackGroundPixel)
#else  /* __STDC__ */
void                GifPrintImage (fn, pres, xif, yif, wif, hif, PicXArea, PicYArea, PicWArea, PicHArea, fd, BackGroundPixel)
char               *fn;
PictureScaling           pres;
int                 xif;
int                 yif;
