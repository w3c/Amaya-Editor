...
void filter_button_func (GtkWidget *widget, gpointer data)
{
  ...
  fs = (GtkMyFileSelection *)data;
  dirname = cmpl_reference_position (fs->cmpl_state);
  ...
  if (GTK_TOGGLE_BUTTON (button)->active) 
    {
      ...
      gtk_widget_set_sensitive ((GTK_MY_FILE_SELECTION (user_data))->transform_URL_checkbutton,FALSE);
    }
  else
    {
      gtk_widget_set_sensitive ((GTK_MY_FILE_SELECTION (user_data))->copy_image_checkbutton, TRUE);
...
static void gtk_my_file_selection_class_init    (GtkMyFileSelectionClass *klass);
static void gtk_my_file_selection_init          (GtkMyFileSelection      *filesel);
static void gtk_my_file_selection_destroy       (GtkObject             *object);
static gint gtk_my_file_selection_key_press     (GtkWidget             *widget,
					      GdkEventKey           *event,
					      gpointer               user_data);

static void gtk_my_file_selection_file_button (GtkWidget *widget,
					    gint row, 
					    gint column, 
					    GdkEventButton *bevent,
					    gpointer user_data);

static void gtk_my_file_selection_dir_button (GtkWidget *widget,
					   gint row, 
					   gint column, 
					   GdkEventButton *bevent,
					   gpointer data);

static void gtk_my_file_selection_populate      (GtkMyFileSelection      *fs,
					      gchar                 *rel_path,
					      gint                   try_complete);
static void gtk_my_file_selection_abort         (GtkMyFileSelection      *fs);

static void gtk_my_file_selection_update_history_menu (GtkMyFileSelection       *fs,
						    gchar                  *current_dir);

static void gtk_my_file_selection_create_dir (GtkWidget *widget, gpointer data);
static void gtk_my_file_selection_delete_file (GtkWidget *widget, gpointer data);
static void gtk_my_file_selection_rename_file (GtkWidget *widget, gpointer data);
...
GtkType
gtk_my_file_selection_get_type (void)
{
  ...
  if (!file_selection_type)
    {
      ...
      {
	"GtkMyFileSelection",
	sizeof (GtkMyFileSelection),
	sizeof (GtkMyFileSelectionClass),
	(GtkClassInitFunc) gtk_my_file_selection_class_init,
	(GtkObjectInitFunc) gtk_my_file_selection_init,
	/* reserved_1 */ NULL,
	/* reserved_2 */ NULL,
        (GtkClassInitFunc) NULL,
      };

      file_selection_type = gtk_type_unique (GTK_TYPE_FRAME, &filesel_info);
    }
...
static void
gtk_my_file_selection_class_init (GtkMyFileSelectionClass *class)
{
  ...
  object_class->destroy = gtk_my_file_selection_destroy;
}

static void
new_background_image_file_selection_init (GtkMyFileSelection *filesel)
{
  ...
  GtkWidget *back_image_frame;
  GtkWidget *back_image_hbox;
  GSList *back_image_hbox_group = NULL;
  ...
  gtk_my_file_selection_show_fileop_buttons(filesel);
  ...
  gtk_signal_connect (GTK_OBJECT (filesel->dir_list), "select_row",
		      (GtkSignalFunc) gtk_my_file_selection_dir_button, 
		      (gpointer) filesel);
  ...
  gtk_signal_connect (GTK_OBJECT (filesel->file_list), "select_row",
		      (GtkSignalFunc) gtk_my_file_selection_file_button, 
		      (gpointer) filesel);
  ...
  gtk_button_box_set_layout(GTK_BUTTON_BOX(confirm_area),GTK_BUTTONBOX_SPREAD);
  ...
  /************************/
  /* The clear button */
  filesel->clear_button = gtk_button_new_with_label ("Clear");
  GTK_WIDGET_SET_FLAGS (filesel->clear_button, GTK_CAN_DEFAULT);
  gtk_box_pack_start (GTK_BOX (confirm_area), filesel->clear_button, TRUE, TRUE, 0);
  gtk_widget_show (filesel->clear_button);
  gtk_signal_connect (GTK_OBJECT (filesel->clear_button), "clicked",
		      (GtkSignalFunc)back_clear_button_func, filesel);
  /* The filter button */
  filesel->filter_button = gtk_button_new_with_label ("Filter");
  GTK_WIDGET_SET_FLAGS (filesel->filter_button, GTK_CAN_DEFAULT);
  gtk_box_pack_start (GTK_BOX (confirm_area), filesel->filter_button, TRUE, TRUE, 0);
  gtk_widget_show (filesel->filter_button);
  gtk_signal_connect (GTK_OBJECT (filesel->filter_button), "clicked",
		      (GtkSignalFunc)filter_button_func, (gpointer) filesel);
  ...
  gtk_signal_connect (GTK_OBJECT (filesel->cancel_button), "clicked",
		      (GtkSignalFunc)cancel_button_func, dialog_box);
  ...
  /*****************************************/
  /* rajout pour new html document */ 
  filesel->filter_label = gtk_label_new ("Filter");
  gtk_misc_set_alignment (GTK_MISC (filesel->filter_label), 0.0, 0.5);
  gtk_box_pack_start (GTK_BOX (entry_vbox), filesel->filter_label, 
		      FALSE, FALSE, 0);
  gtk_widget_show (filesel->filter_label);

  filter_hbox  = gtk_hbox_new (FALSE,5);
  gtk_box_pack_start (GTK_BOX (entry_vbox),filter_hbox , FALSE, FALSE, 0);
  gtk_widget_show (filter_hbox);

  filesel->filter_entry = gtk_entry_new ();
  gtk_box_pack_start (GTK_BOX (filter_hbox), filesel->filter_entry, 
		      FALSE, FALSE, 0);
  gtk_widget_set_usize (filesel->filter_entry, 100, -1);
  gtk_entry_set_text (GTK_ENTRY(filesel->filter_entry), ".html");
  gtk_widget_show (filesel->filter_entry);
  gtk_signal_connect (GTK_OBJECT (filesel->filter_entry), "changed",
		      (GtkSignalFunc) back_filter_entry_func, 
		      NULL);
  label = gtk_label_new ("");
   gtk_box_pack_start (GTK_BOX (filter_hbox), label, TRUE, TRUE, 0);
  gtk_widget_show (label);

  /****************************************/
  temphbox = gtk_hbox_new (FALSE, 0);
  gtk_box_pack_start (GTK_BOX (entry_vbox), temphbox, FALSE, FALSE, 0);
  gtk_widget_show (temphbox);
 
    filesel->address_label = gtk_label_new ("Background image");
  gtk_misc_set_alignment (GTK_MISC (filesel->address_label), 0.0, 0.5);
  gtk_box_pack_start (GTK_BOX (temphbox), filesel->address_label, FALSE, FALSE, 0);
  gtk_widget_show (filesel->address_label);
  ...
  gtk_signal_connect (GTK_OBJECT (filesel->selection_entry), "key_press_event",
		      (GtkSignalFunc) gtk_my_file_selection_key_press, filesel);
  ...
  if (!cmpl_state_okay (filesel->cmpl_state))
    ...
    {
      gtk_my_file_selection_populate (filesel, "", FALSE);
    }
  ...
  /**************************/
  back_image_frame = gtk_frame_new ("Background image");
  gtk_widget_show (back_image_frame);
  gtk_box_pack_start (GTK_BOX (entry_vbox), back_image_frame, FALSE, TRUE, 5);

  back_image_hbox = gtk_hbox_new (TRUE, 5);
  gtk_widget_show (back_image_hbox);
  gtk_container_add (GTK_CONTAINER (back_image_frame), back_image_hbox);

  filesel->repeat_radiobutton = gtk_radio_button_new_with_label (back_image_hbox_group, "repeat");
  back_image_hbox_group = gtk_radio_button_group (GTK_RADIO_BUTTON (filesel->repeat_radiobutton));
  gtk_widget_show (filesel->repeat_radiobutton);
  gtk_box_pack_start (GTK_BOX (back_image_hbox), filesel->repeat_radiobutton, TRUE, TRUE, 2);
  gtk_signal_connect (GTK_OBJECT (filesel->repeat_radiobutton), "clicked",
                      GTK_SIGNAL_FUNC (repeat_func),
                      NULL);

  filesel->repeat_x_radiobutton = gtk_radio_button_new_with_label (back_image_hbox_group, "repeat-x");
  back_image_hbox_group = gtk_radio_button_group (GTK_RADIO_BUTTON (filesel->repeat_x_radiobutton));
  gtk_widget_show (filesel->repeat_x_radiobutton);
  gtk_box_pack_start (GTK_BOX (back_image_hbox), filesel->repeat_x_radiobutton, TRUE, TRUE, 2);
  gtk_signal_connect (GTK_OBJECT (filesel->repeat_x_radiobutton), "clicked",
                      GTK_SIGNAL_FUNC (repeat_x_func),
                      NULL);

  filesel->repeat_y_radiobutton = gtk_radio_button_new_with_label (back_image_hbox_group, "repeat-y");
  back_image_hbox_group = gtk_radio_button_group (GTK_RADIO_BUTTON (filesel->repeat_y_radiobutton));
  gtk_widget_show (filesel->repeat_y_radiobutton);
  gtk_box_pack_start (GTK_BOX (back_image_hbox), filesel->repeat_y_radiobutton, TRUE, TRUE, 2);
  gtk_signal_connect (GTK_OBJECT (filesel->repeat_y_radiobutton), "clicked",
                      GTK_SIGNAL_FUNC (repeat_y_func),
                      NULL);

  filesel->no_repeat_radiobutton = gtk_radio_button_new_with_label (back_image_hbox_group, "no_repeat");
  back_image_hbox_group = gtk_radio_button_group (GTK_RADIO_BUTTON (filesel->no_repeat_radiobutton));
  gtk_widget_show (filesel->no_repeat_radiobutton);
  gtk_box_pack_start (GTK_BOX (back_image_hbox), filesel->no_repeat_radiobutton, TRUE, TRUE, 2);
  gtk_signal_connect (GTK_OBJECT (filesel->no_repeat_radiobutton), "clicked",
                      GTK_SIGNAL_FUNC (no_repeat_func),
                      NULL);
}

static void
new_file_selection_init (GtkMyFileSelection *filesel)
{
  GtkWidget *entry_vbox;
  GtkWidget *label;
  GtkWidget *list_hbox;
  ...
  /*  The OK button  */
  filesel->ok_button = gtk_button_new_with_label ("Confirm");
  GTK_WIDGET_SET_FLAGS (filesel->ok_button, GTK_CAN_DEFAULT);
  gtk_box_pack_start (GTK_BOX (confirm_area), filesel->ok_button, TRUE, TRUE, 0);
  gtk_widget_grab_default (filesel->ok_button);
  gtk_widget_show (filesel->ok_button);
 /* gtk_signal_connect (GTK_OBJECT (filesel->ok_button), "clicked",
		      (GtkSignalFunc)confirm_button_func, NULL);*/
  /* The clear button */
  filesel->clear_button = gtk_button_new_with_label ("Clear");
  GTK_WIDGET_SET_FLAGS (filesel->clear_button, GTK_CAN_DEFAULT);
  gtk_box_pack_start (GTK_BOX (confirm_area), filesel->clear_button, TRUE, TRUE, 0);
  gtk_widget_show (filesel->clear_button);
  gtk_signal_connect (GTK_OBJECT (filesel->clear_button), "clicked",
		      (GtkSignalFunc)new_clear_button_func, filesel);
  /* The filter button */
  filesel->filter_button = gtk_button_new_with_label ("Filter");
  GTK_WIDGET_SET_FLAGS (filesel->filter_button, GTK_CAN_DEFAULT);
  gtk_box_pack_start (GTK_BOX (confirm_area), filesel->filter_button, TRUE, TRUE, 0);
  ...
  gtk_widget_show (filesel->filter_entry);
  gtk_signal_connect (GTK_OBJECT (filesel->filter_entry), "changed",
		      (GtkSignalFunc) new_filter_entry_func, 
		      NULL);
  label = gtk_label_new ("");
  gtk_box_pack_start (GTK_BOX (filter_hbox), label, TRUE, TRUE, 0);
  gtk_widget_show (label);
  ...
  filesel->address_label = gtk_label_new ("Address");
  gtk_misc_set_alignment (GTK_MISC (filesel->address_label), 0.0, 0.5);
  gtk_box_pack_start (GTK_BOX (temphbox), filesel->address_label, FALSE, FALSE, 0);
  gtk_widget_show (filesel->address_label);
...
/* function that creates the "save as"  dialog box */
static void save_file_selection_init (GtkMyFileSelection *filesel)
{
  GtkWidget *entry_vbox;
  GtkWidget *label;
  GtkWidget *list_hbox;
  ...
  GtkWidget *pulldown_hbox;
  GtkWidget *scrolled_win;
  GtkWidget *doc_format_frame;
  GtkWidget *doc_format_hbox;
  GtkWidget *doc_format_vbox;
  GtkWidget *doc_format_vbox2;
  ...
  /* The document format frame */
  doc_format_frame = gtk_frame_new ("Document format");
  gtk_widget_show (doc_format_frame);
  gtk_box_pack_start (GTK_BOX (filesel->main_vbox), doc_format_frame, FALSE, FALSE, 0);

  doc_format_hbox = gtk_hbox_new (TRUE, 0);
  gtk_widget_show (doc_format_hbox);
  gtk_container_add (GTK_CONTAINER (doc_format_frame), doc_format_hbox);

  doc_format_vbox = gtk_vbox_new (FALSE, 0);
  gtk_widget_show (doc_format_vbox);
  gtk_box_pack_start (GTK_BOX (doc_format_hbox), doc_format_vbox, TRUE, TRUE, 0);

  filesel->saved_format = NULL;
 
  /* The "html" radiobutton*/
  filesel->html_radiobutton = gtk_radio_button_new_with_label (filesel->saved_format, "HTML");
  gtk_widget_show (filesel->html_radiobutton);
  gtk_box_pack_start (GTK_BOX (doc_format_vbox), filesel->html_radiobutton, TRUE, TRUE, 0);
  gtk_signal_connect (GTK_OBJECT (filesel->html_radiobutton), "clicked",
                      GTK_SIGNAL_FUNC (html_format_func),
                      NULL);
  filesel->saved_format = gtk_radio_button_group (GTK_RADIO_BUTTON (filesel->html_radiobutton));
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (filesel->html_radiobutton), TRUE);  
 
  /* The "xhtml" radiobutton*/
  filesel->xhtml_radiobutton = gtk_radio_button_new_with_label (filesel->saved_format,"XHTML");
  gtk_widget_show (filesel->xhtml_radiobutton);
  gtk_box_pack_start (GTK_BOX (doc_format_vbox), filesel->xhtml_radiobutton, TRUE, TRUE, 0);
  gtk_signal_connect (GTK_OBJECT (filesel->xhtml_radiobutton), "clicked",
                      GTK_SIGNAL_FUNC (xhtml_format_func),
                      NULL);
  filesel->saved_format = gtk_radio_button_group (GTK_RADIO_BUTTON (filesel->xhtml_radiobutton));

  /* The "text" radiobutton*/
  filesel->text_radiobutton = gtk_radio_button_new_with_label (filesel->saved_format,"Text");
  gtk_widget_show (filesel->text_radiobutton);
  gtk_box_pack_start (GTK_BOX (doc_format_vbox), filesel->text_radiobutton, TRUE, TRUE, 0);
  gtk_signal_connect (GTK_OBJECT (filesel->text_radiobutton), "clicked",
                      GTK_SIGNAL_FUNC (text_format_func),
                      filesel);
  filesel->saved_format = gtk_radio_button_group (GTK_RADIO_BUTTON (filesel->text_radiobutton));

  doc_format_vbox2 = gtk_vbox_new (FALSE, 0);
  gtk_widget_show (doc_format_vbox2);
  gtk_box_pack_start (GTK_BOX (doc_format_hbox), doc_format_vbox2, TRUE, TRUE, 0);

  /* The "copy images" checkbutton*/
  filesel->copy_image_checkbutton = gtk_check_button_new_with_label ("Copy images");
  gtk_widget_show (filesel->copy_image_checkbutton);
  gtk_box_pack_start (GTK_BOX (doc_format_vbox2), filesel->copy_image_checkbutton, TRUE, TRUE, 0);
  gtk_signal_connect (GTK_OBJECT (filesel->copy_image_checkbutton), "clicked",
                      GTK_SIGNAL_FUNC (copy_images_func),
                      NULL);
  /* The "transform URL" checkbutton*/
  filesel->transform_URL_checkbutton = gtk_check_button_new_with_label ("Transform URL's");
  gtk_widget_show (filesel->transform_URL_checkbutton);
  gtk_box_pack_start (GTK_BOX (doc_format_vbox2), filesel->transform_URL_checkbutton, TRUE, TRUE, 0);
  gtk_signal_connect (GTK_OBJECT (filesel->transform_URL_checkbutton), "clicked",
                      GTK_SIGNAL_FUNC (transform_url_func),
                      NULL);
  ...
  confirm_area = gtk_hbutton_box_new ();
  gtk_button_box_set_layout(GTK_BUTTON_BOX(confirm_area),GTK_BUTTONBOX_SPREAD);
  gtk_button_box_set_spacing(GTK_BUTTON_BOX(confirm_area),0);
  gtk_box_pack_end (GTK_BOX (filesel->main_vbox), confirm_area, FALSE, FALSE, 0);
  gtk_widget_show (confirm_area);
  ...
  gtk_widget_show (filesel->clear_button);
  gtk_signal_connect (GTK_OBJECT (filesel->clear_button), "clicked",
		      (GtkSignalFunc)save_clear_button_func, filesel);
  /* The filter button */
  filesel->filter_button = gtk_button_new_with_label ("Filter");
  GTK_WIDGET_SET_FLAGS (filesel->filter_button, GTK_CAN_DEFAULT);
  gtk_box_pack_start (GTK_BOX (confirm_area), filesel->filter_button, TRUE, TRUE, 0);
  ...
  filesel->filter_hbox  = gtk_hbox_new (FALSE,5);
  gtk_box_pack_start (GTK_BOX (entry_vbox),filesel->filter_hbox , FALSE, FALSE, 0);
  gtk_widget_show (filesel->filter_hbox);

  /* The filter entry widget */
  filesel->filter_entry = gtk_entry_new ();
  gtk_box_pack_start (GTK_BOX (filesel->filter_hbox), filesel->filter_entry, 
		      FALSE, FALSE, 0);
  gtk_widget_set_usize (filesel->filter_entry, 100, -1);
  gtk_entry_set_text (GTK_ENTRY(filesel->filter_entry), ".html");
  gtk_widget_show (filesel->filter_entry);
  gtk_signal_connect (GTK_OBJECT (filesel->filter_entry), "changed",
		      (GtkSignalFunc)save_filter_entry_func, 
		      NULL);
  
  label = gtk_label_new ("");
  gtk_box_pack_start (GTK_BOX (filesel->filter_hbox), label, TRUE, TRUE, 0);
  gtk_widget_show (label);

 
  /* The "Image Directory" label widget */
  filesel->image_directory_label = gtk_label_new ("Image Directory");
  gtk_misc_set_alignment (GTK_MISC (filesel->image_directory_label), 0.0, 0.5);
  gtk_box_pack_start (GTK_BOX (entry_vbox), filesel->image_directory_label, FALSE, FALSE, 0);
  gtk_widget_show (filesel->image_directory_label);
  
  /* The "Image Directory" entry widget */
  filesel->image_directory_entry = gtk_entry_new ();
  gtk_box_pack_start (GTK_BOX (entry_vbox), filesel->image_directory_entry, 
		      FALSE, FALSE, 0);
  gtk_widget_show (filesel->image_directory_entry);
  gtk_signal_connect (GTK_OBJECT (filesel->image_directory_entry), "changed",
		      (GtkSignalFunc) image_dir_entry_func, 
		      NULL);
  ...
  /* The "Document location" label widget */
  filesel->address_label = gtk_label_new ("Document location");
  gtk_misc_set_alignment (GTK_MISC (filesel->address_label), 0.0, 0.5);
  gtk_box_pack_start (GTK_BOX (temphbox), filesel->address_label, FALSE, FALSE, 0);
  gtk_widget_show (filesel->address_label);
  ...
  /* The "Document location" entry widget */
  filesel->selection_entry = gtk_entry_new ();
  gtk_signal_connect (GTK_OBJECT (filesel->selection_entry), "key_press_event",
		      (GtkSignalFunc) gtk_my_file_selection_key_press, filesel);
  gtk_signal_connect_object (GTK_OBJECT (filesel->selection_entry), "focus_in_event",
  ...
  gtk_box_pack_start (GTK_BOX (entry_vbox), filesel->selection_entry, TRUE, TRUE, 0);
  gtk_widget_show (filesel->selection_entry);
  gtk_entry_set_text (GTK_ENTRY (filesel->selection_entry), "http//www.w3.org");
...
static void
gtk_my_file_selection_init (GtkMyFileSelection *filesel)
{
  if (dialog_type == SAVE)
   save_file_selection_init (filesel);
  else if (dialog_type == NEW)
    new_file_selection_init (filesel);
  else if (dialog_type == BACKGROUND_IMAGE)
    new_background_image_file_selection_init (filesel);
  else printf ("erreur dans gtk_my_file_selection_init\n");
}
GtkWidget*
gtk_my_file_selection_new (const gchar *title)
{
  GtkMyFileSelection *filesel;

  filesel = gtk_type_new (GTK_TYPE_MY_FILE_SELECTION);
  gtk_frame_set_label (GTK_FRAME (filesel), title);

  return GTK_WIDGET (filesel);
}

void
gtk_my_file_selection_show_fileop_buttons (GtkMyFileSelection *filesel)
{
  g_return_if_fail (filesel != NULL);
  g_return_if_fail (GTK_IS_MY_FILE_SELECTION (filesel));
  ...
  if (!filesel->fileop_c_dir) 
    {
      ...
      gtk_signal_connect (GTK_OBJECT (filesel->fileop_c_dir), "clicked",
			  (GtkSignalFunc) gtk_my_file_selection_create_dir, 
			  (gpointer) filesel);
  ...
  if (!filesel->fileop_del_file) 
    {
      ...
      gtk_signal_connect (GTK_OBJECT (filesel->fileop_del_file), "clicked",
			  (GtkSignalFunc) gtk_my_file_selection_delete_file, 
			  (gpointer) filesel);
      gtk_box_pack_start (GTK_BOX (filesel->button_area), 
  ...
  if (!filesel->fileop_ren_file)
    {
      ...
      gtk_signal_connect (GTK_OBJECT (filesel->fileop_ren_file), "clicked",
			  (GtkSignalFunc) gtk_my_file_selection_rename_file, 
			  (gpointer) filesel);
      gtk_box_pack_start (GTK_BOX (filesel->button_area), 
...
void       
gtk_my_file_selection_hide_fileop_buttons (GtkMyFileSelection *filesel)
{
  g_return_if_fail (filesel != NULL);
  g_return_if_fail (GTK_IS_MY_FILE_SELECTION (filesel));
...
void
gtk_my_file_selection_set_filename (GtkMyFileSelection *filesel,
				 const gchar      *filename)
{
  ...
  gtk_my_file_selection_populate (filesel, buf, FALSE);
...
gchar*
gtk_my_file_selection_get_filename (GtkMyFileSelection *filesel)
{
  ...
  g_return_val_if_fail (GTK_IS_MY_FILE_SELECTION (filesel), nothing);
...
void
gtk_my_file_selection_complete (GtkMyFileSelection *filesel,
			     const gchar      *pattern)
{
  g_return_if_fail (filesel != NULL);
  g_return_if_fail (GTK_IS_MY_FILE_SELECTION (filesel));
  ...
  gtk_my_file_selection_populate (filesel, (gchar*) pattern, TRUE);
}

static void
gtk_my_file_selection_destroy (GtkObject *object)
{
  GtkMyFileSelection *filesel;
  GList *list;
  ...
  g_return_if_fail (GTK_IS_MY_FILE_SELECTION (object));

  filesel = GTK_MY_FILE_SELECTION (object);
...
static void
gtk_my_file_selection_fileop_error (gchar *error_message)
{
  GtkWidget *label;
  GtkWidget *vbox;
  GtkWidget *button;
  ...
  gtk_signal_connect (GTK_OBJECT (dialog), "destroy",
		      (GtkSignalFunc) gtk_my_file_selection_fileop_destroy, 
		      (gpointer) fs);
  */
...
static void
gtk_my_file_selection_fileop_destroy (GtkWidget *widget, gpointer data)
{
  GtkMyFileSelection *fs = data;

  g_return_if_fail (fs != NULL);
  g_return_if_fail (GTK_IS_MY_FILE_SELECTION (fs));
...
static void
gtk_my_file_selection_create_dir_confirmed (GtkWidget *widget, gpointer data)
{
  GtkMyFileSelection *fs = data;
  gchar *dirname;
  gchar *path;
  ...
  if ( (mkdir (full_path, 0755) < 0) ) 
    {
      ...
      gtk_my_file_selection_fileop_error (buf);
    }
  g_free (full_path);
  
  gtk_widget_destroy (fs->fileop_dialog);
  gtk_my_file_selection_populate (fs, "", FALSE);
}
  
static void
gtk_my_file_selection_create_dir (GtkWidget *widget, gpointer data)
{
  GtkMyFileSelection *fs = data;
  GtkWidget *label;
  GtkWidget *dialog;
  ...
  gtk_signal_connect (GTK_OBJECT (button), "clicked",
		      (GtkSignalFunc) gtk_my_file_selection_create_dir_confirmed, 
		      (gpointer) fs);
  gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialog)->action_area),
		     button, TRUE, TRUE, 0);
  GTK_WIDGET_SET_FLAGS(button, GTK_CAN_DEFAULT);
...
static void
gtk_my_file_selection_delete_file_confirmed (GtkWidget *widget, gpointer data)
{
  GtkMyFileSelection *fs = data;
  CompletionState *cmpl_state;
  gchar *path;
...
static void
gtk_my_file_selection_delete_file (GtkWidget *widget, gpointer data)
{
  GtkMyFileSelection *fs = data;
  GtkWidget *label;
  GtkWidget *vbox;
  ...
  gtk_signal_connect (GTK_OBJECT (button), "clicked",
		      (GtkSignalFunc) gtk_my_file_selection_delete_file_confirmed, 
		      (gpointer) fs);
  gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialog)->action_area),
		     button, TRUE, TRUE, 0);
  GTK_WIDGET_SET_FLAGS(button, GTK_CAN_DEFAULT);
...
static void
gtk_my_file_selection_rename_file_confirmed (GtkWidget *widget, gpointer data)
{
  GtkMyFileSelection *fs = data;
  gchar *buf;
  gchar *file;
...
static void
gtk_my_file_selection_rename_file (GtkWidget *widget, gpointer data)
{
  GtkMyFileSelection *fs = data;
  GtkWidget *label;
  GtkWidget *dialog;
  ...
  gtk_signal_connect (GTK_OBJECT (button), "clicked",
		      (GtkSignalFunc) gtk_my_file_selection_rename_file_confirmed, 
		      (gpointer) fs);
  gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialog)->action_area),
		     button, TRUE, TRUE, 0);
  GTK_WIDGET_SET_FLAGS(button, GTK_CAN_DEFAULT);
...
static gint
gtk_my_file_selection_key_press (GtkWidget   *widget,
			      GdkEventKey *event,
			      gpointer     user_data)
{
  GtkMyFileSelection *fs;
  ...
  if (event->keyval == GDK_Tab)
    {
      fs = GTK_MY_FILE_SELECTION (user_data);
      text = gtk_entry_get_text (GTK_ENTRY (fs->selection_entry));

      text = g_strdup (text);

      gtk_my_file_selection_populate (fs, text, TRUE);
...
static void
gtk_my_file_selection_history_callback (GtkWidget *widget, gpointer data)
{
  GtkMyFileSelection *fs = data;
  HistoryCallbackArg *callback_arg;
  GList *list;
  ...
  while (list) {
    ...
    if (callback_arg->menu_item == widget)
      {
	gtk_my_file_selection_populate (fs, callback_arg->directory, FALSE);
	break;
      }
...
static void 
gtk_my_file_selection_update_history_menu (GtkMyFileSelection *fs,
  ...
  for (i = dir_len; i >= 0; i--)
      ...
      if ( (current_dir[i] == '/') || (i == dir_len))
	{
	  ...
	  gtk_signal_connect (GTK_OBJECT (menu_item), "activate",
			      (GtkSignalFunc) gtk_my_file_selection_history_callback,
			      (gpointer) fs);
...
static void
gtk_my_file_selection_file_button (GtkWidget *widget,
			       gint row, 
			       gint column, 
			       GdkEventButton *bevent,
			       gpointer user_data)
{
  GtkMyFileSelection *fs = NULL;
...
static void
gtk_my_file_selection_dir_button (GtkWidget *widget,
			       gint row, 
			       gint column, 
			       GdkEventButton *bevent,
			       gpointer user_data)
{
  ...
  if (filename)
	...
	switch (bevent->type)
	  {
	  case GDK_2BUTTON_PRESS:
	    gtk_my_file_selection_populate (fs, filename, FALSE);
	    break;
...
static void
gtk_my_file_selection_populate (GtkMyFileSelection *fs,
			     gchar            *rel_path,
			     gint              try_complete)
{
  CompletionState *cmpl_state;
  ...
  if (!cmpl_state_okay (cmpl_state))
    {
      ...
      gtk_my_file_selection_abort (fs);
      return;
    }
  ...
  while (poss)
    {
	  ...
	  text[0] = filename;
                  ...
                  strcmp (filename, "../") != 0)
		{
          ...
          else if (strstr(text[0],suffix)!=NULL)
      ...
      if (cmpl_updated_text (cmpl_state)[0])
        {
	      ...
	      gchar* dir_name = g_strdup (cmpl_updated_text (cmpl_state));
              ...
              gtk_my_file_selection_populate (fs, dir_name, TRUE);
...
//	gtk_entry_set_text (GTK_ENTRY (fs->selection_entry), "");
  ...
  if (!did_recurse)
	...
	{
	  gtk_my_file_selection_update_history_menu (fs, cmpl_reference_position (cmpl_state));
	}
...
static void
gtk_my_file_selection_abort (GtkMyFileSelection *fs)
{
  gchar err_buf[256];
